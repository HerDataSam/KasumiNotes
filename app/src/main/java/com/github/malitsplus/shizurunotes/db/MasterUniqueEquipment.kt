                ...
                for (i in 1..10) {
                    ...
                    if (itemId in 140000..140001) {
                        DBHelper.get().getEquipmentPiece(itemId)?.let { piece ->
                            map[EquipmentPiece(piece.equipment_id, piece.equipment_name, piece.description, piece.equipment_id / 10 * 10)] = Utils.getValueFromObject(it, "consume_num_$i") as Int
                        }
                    } else if (itemId in 25000..39999) {
                        DBHelper.get().getItemData(itemId)?.let{ item ->
                            map[GeneralItem(item.item_id, item.item_name, ItemType.GENERAL_ITEM, item.description, item.item_id)] = Utils.getValueFromObject(it, "consume_num_$i") as Int
                        }
                    }
                }
                craftMap = map
